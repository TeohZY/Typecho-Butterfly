...
<?php
// 密码保护机制 - 使用 Session 和哈希验证
session_start();

if (isset($_POST['pwd'])) {
    $inputPwd = trim($_POST['pwd']);
    $savedPwd = $this->options->ThemePassword;

    // 使用哈希验证密码，防止明文比较
    if (!empty($savedPwd) && hash_equals(hash('sha256', $inputPwd), hash('sha256', $savedPwd))) {
        // 密码正确，存储到 session
        $_SESSION['ThemeAccess'] = true;
        echo json_encode(['status' => '200', 'info' => '密码正确,芝麻开门！']);
        exit;
    } else {
        echo json_encode(['status' => '403', 'info' => '密码错误']);
        exit;
    }
}

// 检查是否已授权
$isAuthorized = isset($_SESSION['ThemeAccess']) && $_SESSION['ThemeAccess'] === true;

if (!empty($storedPwd) && ($this->options->Defend === 'on') && !$isAuthorized) {
?>
