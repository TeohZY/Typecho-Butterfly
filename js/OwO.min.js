...
function lazyImg(text) {
    const pattern = /<img[^>]*src=['"]([^'"]+)['"][^>]*(alt=['"]([^'"]*)['"])?[^>]*(style=['"][^'"]+['"])?[^>]*(class=['"]([^'"]*)['"])?[^>]*>/gi;
    const replacement = '<img title="$3" alt="$3" data-lazy-src="$1" $4 class="lazyload $6" src="' + getLazyLoad() + '">';

    return text.replace(pattern, (match, src, altPart, alt, style, classPart, existingClass) => {
        return `<img title="${alt || ''}" alt="${alt || ''}" data-lazy-src="${src}" ${style || ''} class="lazyload ${existingClass || ''}" src="${getLazyLoad()}">`;
    });
}
    ...
    var e = function () {
        ...
        return _createClass(e, [{
            ...
            value: function (e) {
                ...
                for (var a = '\n            <div class="OwO-logo"><span>' + e.logo + '</span></div>\n            <div class="OwO-body" style="width: ' + e.width + '">', s = 0; s < this.packages.length; s++) {
                    ...
                    for (var n = this.odata[this.packages[s]].container, i = 0; i < n.length; i++)
                        a += '\n                    <li class="OwO-item" data-type="' + this.odata[this.packages[s]].type + '" data-key="' + this.packages[s] + '" title="' + n[i].text + '">' + lazyImg(n[i].icon) + "</li>";
                    a += "\n                </ul>"
                }
                    ...
                    this.container.getElementsByClassName("OwO-body")[0].addEventListener("click", function (e) {
                        ...
                        if (e.target.classList.contains("OwO-item") ? a = e.target : e.target.parentNode.classList.contains("OwO-item") && (a = e.target.parentNode),
                            ...
                            var s = t.area.selectionEnd
                                , n = t.area.value, type = a.dataset.type,
                                key = a.dataset.key, text = a.title;
                            if (type == 'image') {
                                t.area.value = n.slice(0, s) + `{% icon ${key},${text} %}` + n.slice(s)
                            } else {
            ...
            value: function (e) {
                ...
                const newBlock = this.container.getElementsByClassName("OwO-items")[e];
                newBlock.classList.add("OwO-items-show");

                // 手动触发新显示块中图片的懒加载
                newBlock.querySelectorAll('img.lazyload').forEach(img => {
                    if (img.getAttribute('data-lazy-src')) {
                        img.src = img.getAttribute('data-lazy-src');
                        img.classList.add('entered'); // 标记为已加载，避免重复触发
                        img.classList.add('loaded'); // 标记为已加载，避免重复触发
                    }
                });
