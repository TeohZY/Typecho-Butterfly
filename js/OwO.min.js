...
function lazyImg(text) {
    ...
    const pattern = /<img[^>]*src=['"]([^'"]+)['"][^>]*(alt=['"]([^'"]*)['"])?[^>]*(style=['"][^'"]+['"])?[^>]*>/gi;
    const replacement = '<img title="$3" alt="$3" data-lazy-src="$1" $4 src="' + getLazyLoad() + '">';
    
    return text.replace(pattern, (match, src, altPart, alt, style) => {
        console.log(`<img title="${alt || ''}" alt="${alt || ''}" data-lazy-src="${src}" ${style || ''} src="${getLazyLoad()}">`);
        
        return `<img title="${alt || ''}" alt="${alt || ''}" data-lazy-src="${src}" ${style || ''} src="${getLazyLoad()}">`;
    });
}
